...
% Simulate a VAR model estimated on the data from Hansen (2016) with
...
if ~isempty(getenv('SLURM_JOB_CPUS_PER_NODE'))
    ...
    sprintf("[Cluster] SLURM_JOB_CPUS_PER_NODE: %d\n\n", num_workers)
...
[IRF_true, sIRF_true] = varmaIRF(A, zeros(7), P, H);
...
if FLAG_write_latex
    ...
    LaTeX_Cover_tab.tableLabel = 'tab:ir_cover_1';
    LaTeX_Cover_tab.tableBorders = false;
    LaTeX_Cover_tab.booktabs = true;
    
    fid = fopen('simulation__s_irf1_cover.tex', 'w');
...
for j = 1:K
    tab = [tab; ...
        res(1).Length(j, [2, 1+(4:4:24)]) * 100; ...
        res(2).Length(j, [2, 1+(4:4:24)]) * 100; ...
        res(3).Length(j, [2, 1+(4:4:24)]) * 100; ...
        res(4).Length(j, [2, 1+(4:4:24)]) * 100; ...
        res(5).Length(j, [2, 1+(4:4:24)]) * 100; ...
        res(6).Length(j, [2, 1+(4:4:24)]) * 100; ...
    ];
end
tab = [kron(1:K, ones(1,length(methods)))', tab];
...
if FLAG_write_latex
    ...
    LaTeX_Length_tab.tableLabel = 'tab:ir_length_1';
    LaTeX_Length_tab.tableBorders = false;
    LaTeX_Length_tab.booktabs = true;
    
    fid = fopen('simulation__s_irf1_length.tex', 'w');
