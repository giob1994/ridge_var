function [IRF,Std,P,sIRF,sStd,sCov,A,Sig_A,U] = IRFs_ridgeVAR_mod_asy(Yt, p, L, q, H_irf)
...
% A = Y * Z' / (Z * Z' + kron(diag(L), eye(K)));  % w/out intercept
Lm = diag([0; kron(L(:), ones(K,1))]);
B = Y * Z' / (Z * Z' + Lm);
A = B(:, 2:end);       % w/out intercept coeffs

% Modified asymptotic variance factor
L_mod = [zeros(q,1); L(q+1:end)];
Lm_mod = diag([0; kron(L_mod(:), ones(K,1))]);
Gam_mod = (Z*Z' + T * Lm_mod);    % modified Gamma 
Gam_mod = (Gam_mod \ (Z*Z') / Gam_mod)*T;       % asymptotic var. term 

% Covariance estimation
U = Y - B * Z;                          % Residual matrix
Sig_U = U * U' / (T-p-K*p-1);           % Estimated variance
Sig_B = kron(Gam_mod, Sig_U);           % Estimated varcov. of vec(B)
